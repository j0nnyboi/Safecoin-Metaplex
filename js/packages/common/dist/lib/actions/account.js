...
function ensureSplAccount(instructions, cleanupInstructions, toCheck, payer, amount, signers) {
    ...
    instructions.push(safe_token_1.Token.createInitAccountInstruction(ids_1.TOKEN_PROGRAM_ID, ids_1.WRAPPED_SOL_MINT, account, payer));
    cleanupInstructions.push(safe_token_1.Token.createCloseAccountInstruction(ids_1.TOKEN_PROGRAM_ID, account, payer, payer, []));
    return account;
}
...
function createUninitializedAccount(instructions, payer, amount, signers) {
    ...
    instructions.push(web3_js_1.SystemProgram.createAccount({
        ...
        newAccountPubkey: account.publicKey,
        lamports: amount,
        space: safe_token_1.AccountLayout.span,
        programId: ids_1.TOKEN_PROGRAM_ID,
    }));
    signers.push(account);
    return account.publicKey;
...
function createMint(instructions, payer, mintRentExempt, decimals, owner, freezeAuthority, signers) {
    ...
    instructions.push(safe_token_1.Token.createInitMintInstruction(ids_1.TOKEN_PROGRAM_ID, account, decimals, owner, freezeAuthority));
    return account;
}
...
function createTokenAccount(instructions, payer, accountRentExempt, mint, owner, signers) {
    ...
    instructions.push(safe_token_1.Token.createInitAccountInstruction(ids_1.TOKEN_PROGRAM_ID, mint, account, owner));
    return account;
}
...
function ensureWrappedAccount(instructions, cleanupInstructions, toCheck, payer, amount, signers) {
    ...
    instructions.push(safe_token_1.Token.createInitAccountInstruction(TOKEN_PROGRAM_ID, ids_1.WRAPPED_SOL_MINT, account.publicKey, payer));
    cleanupInstructions.push(safe_token_1.Token.createCloseAccountInstruction(TOKEN_PROGRAM_ID, account.publicKey, payer, payer, []));
    signers.push(account);
...
signers, excluded) {
    ...
    if (account && !isWrappedSol) {
        ...
        if (isWrappedSol) {
            cleanupInstructions.push(safe_token_1.Token.createCloseAccountInstruction(ids_1.TOKEN_PROGRAM_ID, toAccount, payer, payer, []));
        }
    }
    return toAccount;
}
