...
programCommand('create_candy_machine')
  .option(
    '-p, --price <string>',
    'Price denominated in SOL or safe-token override',
    '1',
  )
  .option(
    '-t, --safe-token <string>',
  ...
  .option(
    '-a, --safe-token-account <string>',
    'SPL token account that receives mint payments. Only required if safe-token is specified.',
  )
  .option(
