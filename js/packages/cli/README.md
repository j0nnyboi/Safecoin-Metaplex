...
```
metaplex upload ~/nft-test/mini_drop --keypair ~/.config/safecoin/id.json
ts-node cli upload ~/nft-test/mini_drop --keypair ~/.config/safecoin/id.json
```

2. Verify everything is uploaded. Rerun the first command until it is.

```
metaplex verify --keypair ~/.config/safecoin/id.json
ts-node cli verify --keypair ~/.config/safecoin/id.json
```

3. Create your candy machine. It can cost up to ~15 safecoin per 10,000 images.

```
metaplex create_candy_machine -k ~/.config/safecoin/id.json -p 1
ts-node cli create_candy_machine -k ~/.config/safecoin/id.json -p 3
```

4. Set the start date and update the price of your candy machine.

```
metaplex update_candy_machine -k ~/.config/safecoin/id.json -d "20 Apr 2021 04:20:00 GMT" -p 0.1
ts-node cli update_candy_machine -k ~/.config/safecoin/id.json -d "20 Apr 2021 04:20:00 GMT" -p 0.1
```

5. Test mint a token (provided it's after the start date)

```
metaplex mint_one_token -k ~/.config/safecoin/id.json
ts-node cli mint_one_token -k ~/.config/safecoin/id.json
```

6. Test mint multiple tokens 

```
metaplex mint_multiple_tokens -k ~/.config/safecoin/id.json -n 100
ts-node cli mint_multiple_tokens -k ~/.config/safecoin/id.json -n 100
```
...
```
metaplex sign_candy_machine_metadata -k ~/.config/safecoin/id.json
ts-node cli sign_candy_machine_metadata -k ~/.config/safecoin/id.json
```

8. If you wish to sign metadata from another candy machine run with the --cndy flag.

```
metaplex sign_candy_machine_metadata -k ~/.config/safecoin/id.json --cndy CANDY_MACHINE_ADDRESS_HERE
ts-node cli sign_candy_machine_metadata -k ~/.config/safecoin/id.json --cndy CANDY_MACHINE_ADDRESS_HERE
```
