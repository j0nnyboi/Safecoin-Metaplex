...
programCommand('upload')
    ...
    const { storage, nftStorageKey, ipfsInfuraProjectId, number, ipfsInfuraSecret, arweaveJwk, awsS3Bucket, retainAuthority, mutable, batchSize, price, splToken, treasuryWallet, gatekeeper, endSettings, hiddenSettings, whitelistMintSettings, goLiveDate, uuid, } = await (0, various_1.getCandyMachineV2Config)(walletKeyPair, anchorProgram, configPath);
    ...
    if (storage === storage_type_1.StorageType.NftStorage && !nftStorageKey) {
        throw new Error('NftStorage selected as storage option but NftStorage project api key were not provided.');
    }
...
function programCommand(name) {
    return commander_1.program
        ...
        .requiredOption('-k, --keypair <path>', `safecoin wallet location`)
