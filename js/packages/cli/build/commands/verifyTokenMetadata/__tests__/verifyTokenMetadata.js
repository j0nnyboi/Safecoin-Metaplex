...
describe('`metaplex verify_token_metadata`', () => {
    ...
    it('throws on invalid share allocation', () => {
        expect(() => (0, index_1.verifyAggregateShare)([{ address: 'some-safecoin-address', share: 80 }], 'placeholder-manifest-file')).toThrowErrorMatchingInlineSnapshot(`"Creator share for placeholder-manifest-file does not add up to 100, got: 80."`);
    ...
    it('warns when there are inconsistent share allocations', () => {
        ...
        (0, index_1.verifyCreatorCollation)([{ address: 'some-safecoin-address', share: 80 }], collatedCreators, '0.json');
        expect(spy).toHaveBeenCalledTimes(1);
    });
