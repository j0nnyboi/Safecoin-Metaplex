...
describe('`metaplex verify_token_metadata`', function () {
    ...
    it('catches mismatched assets', function () {
        ...
        expect(function () {
            return index_1.verifyTokenMetadata({ files: mismatchedAssets });
    ...
    invalidSchemas.forEach(function (invalidSchema) {
        it("invalidates " + path_1.default.relative(__dirname, invalidSchema), function () {
            expect(function () {
                return index_1.verifyTokenMetadata({
    ...
    it('throws on invalid share allocation', function () {
        expect(function () {
            return index_1.verifyAggregateShare([{ address: 'some-safecoin-address', share: 80 }], 'placeholder-manifest-file');
    ...
    it('warns when there are inconsistent share allocations', function () {
        ...
        index_1.verifyCreatorCollation([{ address: 'some-safecoin-address', share: 80 }], collatedCreators, '0.json');
        expect(spy).toHaveBeenCalledTimes(1);
    });
    it('warns when there are inconsistent creator allocations', function () {
        ...
        index_1.verifyConsistentShares(collatedCreators);
        expect(spy).toHaveBeenCalledTimes(1);
    });
});
