...
async function uploadV2({ files, cacheName, env, totalNFTs, storage, retainAuthority, mutable, nftStorageKey, ipfsCredentials, awsS3Bucket, batchSize, price, treasuryWallet, splToken, gatekeeper, goLiveDate, endSettings, whitelistMintSettings, hiddenSettings, uuid, walletKeyPair, anchorProgram, arweaveJwk, }) {
    var _a, _b;
    ...
    if (!cacheContent.program.uuid) {
        ...
        try {
            ...
            loglevel_1.default.error('Error deploying config to safecoin network.', exx);
            throw exx;
        }
    }
    else {
                    ...
                    try {
                        switch (storage) {
                            case storage_type_1.StorageType.NftStorage:
                                [link, imageLink] = await (0, nft_storage_1.nftStorageUpload)(nftStorageKey, image, manifestBuffer);
                                break;
