    ...
    return __generator(this, function (_e) {
        switch (_e.label) {
            ...
            case 1:
                ...
                return [3 /*break*/, 4];
            case 2:
                token = new safe_token_1.Token(anchorProgram.provider.connection, 
                //@ts-ignore
    ...
    return __generator(this, function (_b) {
        switch (_b.label) {
            ...
            case 2:
                token = (_b.sent())[0];
                instructions = [];
                tokenApp = new safe_token_1.Token(anchorProgram.provider.connection, 
                //@ts-ignore
    ...
    return __generator(this, function (_e) {
        switch (_e.label) {
            ...
            case 2:
                ...
                return [4 /*yield*/, anchorProgram.provider.connection.getMinimumBalanceForRentExemption(safe_token_1.MintLayout.span)];
            case 3:
                instructions = [
        ...
        return __generator(this, function (_e) {
            switch (_e.label) {
                ...
                case 4:
                    transferAuthority = anchor.web3.Keypair.generate();
                    signers.push(transferAuthority);
                    token = new safe_token_1.Token(anchorProgram.provider.connection, fairLaunchObj.treasuryMint, constants_1.TOKEN_PROGRAM_ID, payer);
                    return [4 /*yield*/, token.getMintInfo()];
                case 5:
                    mintInfo = _e.sent();
                    if (adjustMantissa)
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 1:
                    ...
                    return [4 /*yield*/, anchorProgram.provider.connection.getAccountInfo(buyerTokenAccount)];
                case 2:
                    exists = _b.sent();
...
commander_1.program
    ...
    .option('-r, --rpc-url <string>', 'custom rpc url since this is a heavy command')
    .action(function (_, cmd) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, env, keypair, fairLaunch, file, upperTolerance, lowerTolerance, startCursor, endCursor, rpcUrl, walletKeyPair, upTol, lowTol, startC, anchorProgram, fairLaunchKey, fairLaunchObj, array, endC, byCount, i, currSignerBatch, currInstrBatch, sendSigners, sendInstr, ataSignerBatch, ataInstrBatch, ataSigners, ataInstr, txnSize, cursor, lookup, currKey, currCount, existingAta, exists, e_11, myAta, currKey, currCount, existingAta, e_12, txnCursor, i, instructionBatch, signerBatch, e_13, i, instructionBatch, signerBatch, e_14;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _a = cmd.opts(), env = _a.env, keypair = _a.keypair, fairLaunch = _a.fairLaunch, file = _a.file, upperTolerance = _a.upperTolerance, lowerTolerance = _a.lowerTolerance, startCursor = _a.startCursor, endCursor = _a.endCursor, rpcUrl = _a.rpcUrl;
                walletKeyPair = accounts_1.loadWalletKey(keypair);
                upTol = parseInt(upperTolerance);
                lowTol = parseInt(lowerTolerance);
                startC = startCursor ? parseInt(startCursor) : 0;
                return [4 /*yield*/, accounts_1.loadFairLaunchProgram(walletKeyPair, env, rpcUrl)];
            case 1:
                anchorProgram = _b.sent();
                fairLaunchKey = new anchor.web3.PublicKey(fairLaunch);
            ...
            case 2:
                ...
                endC = endCursor ? parseInt(endCursor) : array.length;
                byCount = {};
            ...
            case 3:
                ...
                _b.label = 4;
            case 4:
                if (!(cursor < Math.min(array.length, endC))) return [3 /*break*/, 11];
                currKey = array[cursor];
                currCount = byCount[currKey];
                if (!(currKey.length > 2)) return [3 /*break*/, 9];
                if (!(currCount < lowTol || currCount > upTol)) return [3 /*break*/, 5];
                console.log('Skipped', currKey, 'due to having', currCount, 'allocations.');
                return [3 /*break*/, 9];
            case 5: return [4 /*yield*/, accounts_1.getAtaForMint(fairLaunchObj.tokenMint, new anchor.web3.PublicKey(currKey))];
            case 6:
                existingAta = (_b.sent())[0];
                exists = lookup[existingAta.toBase58()];
                if (!!exists) return [3 /*break*/, 8];
                return [4 /*yield*/, anchorProgram.provider.connection.getAccountInfo(existingAta)];
            case 7:
                exists =
                    !!(_b.sent());
                _b.label = 8;
            case 8:
                if (!exists) {
                    ataInstr.push(instructions_1.createAssociatedTokenAccountInstruction(existingAta, walletKeyPair.publicKey, new anchor.web3.PublicKey(currKey), 
                    //@ts-ignore
                    fairLaunchObj.tokenMint));
            ...
            case 15:
                ...
                _b.label = 16;
            case 16:
                if (!(cursor < Math.min(array.length, endC))) return [3 /*break*/, 21];
                currKey = array[cursor];
                currCount = byCount[currKey];
                if (!(currKey.length > 2)) return [3 /*break*/, 19];
                if (!(currCount < lowTol || currCount > upTol)) return [3 /*break*/, 17];
                console.log('Skipped', currKey, 'due to having', currCount, 'allocations.');
                return [3 /*break*/, 19];
            case 17: return [4 /*yield*/, accounts_1.getAtaForMint(fairLaunchObj.tokenMint, new anchor.web3.PublicKey(currKey))];
            case 18:
                existingAta = (_b.sent())[0];
            ...
            case 24:
                ...
                _b.label = 25;
            case 25:
                if (!(i < ataInstrBatch.length)) return [3 /*break*/, 28];
                instructionBatch = ataInstrBatch[i];
                signerBatch = ataSignerBatch[i];
                return [4 /*yield*/, transactions_1.sendTransactionWithRetryWithKeypair(anchorProgram.provider.connection, walletKeyPair, instructionBatch, signerBatch, 'single')];
            case 26:
                _b.sent();
                txnCursor += txnSize;
                _b.label = 27;
            ...
            case 31:
                ...
                _b.label = 32;
            case 32:
                _b.trys.push([32, 37, , 38]);
                i = 0;
                _b.label = 33;
            case 33:
                if (!(i < currInstrBatch.length)) return [3 /*break*/, 36];
                instructionBatch = currInstrBatch[i];
                signerBatch = currSignerBatch[i];
                return [4 /*yield*/, transactions_1.sendTransactionWithRetryWithKeypair(anchorProgram.provider.connection, walletKeyPair, instructionBatch, signerBatch, 'single')];
            case 34:
                _b.sent();
                txnCursor += txnSize;
        ...
        switch (_b.label) {
            ...
            case 3:
                ...
                return [4 /*yield*/, Promise.all(various_1.chunks(Array.from(Array(tickets.length).keys()), 500).map(function (allIndexesInSlice) { return __awaiter(void 0, void 0, void 0, function () {
                        var i, accountAndPubkey, account, pubkey, state, _a, fairLaunchTicketSeqLookup, seqBump, e_15;
                        return __generator(this, function (_b) {
                            switch (_b.label) {
                                case 0:
                                    i = 0;
