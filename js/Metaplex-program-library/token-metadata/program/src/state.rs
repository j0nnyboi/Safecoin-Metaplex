    ...
    fn add_reservation(
        ...
        while self.reservations.len() < usize_offset {
            self.reservations.push(Reservation {
                address: safecoin_program::system_program::id(),
                spots_remaining: 0,
                total_spots: 0,
            })
        }
        if self.reservations.len() > usize_offset {
            ...
            } else if replaced_addr != safecoin_program::system_program::id() {
        ...
        if usize_offset != 0
            && self.reservations[usize_offset - 1].address == safecoin_program::system_program::id()
        {
