//! Claim bid winnings into a target safe account, only the authorised key can do this, though the
//! target can be any safe account.
...
use crate::{
    ...
    utils::{
        ...
        assert_token_program_matches_package, create_or_allocate_account_raw, safe_token_transfer,
        TokenTransferParams,
    },
    EXTENDED, PREFIX,
};
...
pub fn claim_bid(
    ...
    // Derive Pot address, this account wraps/holds an safe account to transfer tokens into.
    let pot_seeds = [
        PREFIX.as_bytes(),
        program_id.as_ref(),
