...
pub enum AuctionInstruction {
    ...
    ///   2. `[writable]` The pot, containing a reference to the stored safe token account.
    ///   3. `[writable]` The pot safe account, where the tokens will be deposited.
    ...
    /// Move safe tokens from winning bid to the destination account.
