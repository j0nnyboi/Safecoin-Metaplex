...
1. You allocate space for a mint account and a token account using safecoin's system command and then you use spl-token JS SDK `createMint` and `createTokenAccount` commands to create a new mint and a new token account of that mint. You then use the `mintTo` command to mint a single token to that token account.
...
Minting an NFT requires creating a new SPL Mint with the supply of one and decimals zero as described [https://spl.safecoin.com/token#example-create-a-non-fungible-token](https://spl.safecoin.com/token#example-create-a-non-fungible-token)
